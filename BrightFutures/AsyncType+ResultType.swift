...
public extension AsyncType where Value: ResultType {
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recoverWith<E1: ErrorProtocol>(context c: ExecutionContext = DefaultThreadingModel(), task: Value.Error -> Future<Value.Value, E1>) -> Future<Value.Value, E1> {
    ...
    /// The given closure is executed according to the default threading model (see README.md)
    public func mapError<E1: ErrorProtocol>(_ f: Value.Error -> E1) -> Future<Value.Value, E1> {
    ...
    public func mapError<E1: ErrorProtocol>(_ context: ExecutionContext, f: Value.Error -> E1) -> Future<Value.Value, E1> {
        let res = Future<Value.Value, E1>()
...
public extension AsyncType where Value: ResultType, Value.Error == NoError {
    ...
    public func promoteError<E: ErrorProtocol>() -> Future<Value.Value, E> {
...
public extension AsyncType where Value: ResultType, Value.Error == BrightFuturesError<NoError> {
    ...
    public func promoteError<E: ErrorProtocol>() -> Future<Value.Value, BrightFuturesError<E>> {
